...
5. **Update AGENTS.md at project root** (incremental, O(1) operation):
   ...
   - Add to "Active Technologies" section (if not already present):
     - Format: `- [Language + Framework] ([current-branch])`
     - Example: `- Python 3.11 + FastAPI (001-user-auth)`
   - Add to "Recent Changes" section (keep last 3 only):
     - Format: `- [current-branch]: Added [tech stack]`
     - Example: `- 001-user-auth: Added Python 3.11 + FastAPI`
   - Update "Last updated" timestamp to current date
   - Keep file under 150 lines total for token efficiency
   - Preserve any manual additions between `<!-- manual additions start -->` and `<!-- manual additions end -->` markers

**Output**: data-model.md, /contracts/*, failing tests, quickstart.md, updated AGENTS.md
